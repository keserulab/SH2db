    ...
    for gene in table[table["Category"] == cat]["Gene name"].unique():
        for pdb in table[table["Gene name"] == gene]["PDB ID"].unique():
            for chain in table[table["PDB ID"] == pdb]["PDB chain ID"].unique():
                split_pdb = 'pdb_splitchain /home/takacsg/Documents/sh2db/Structures/'+cat+'/'+gene+'/'+pdb+'/'+pdb+'_1.pdb' ## > /home/takacsg/Documents/sh2db/Structures/'+cat+'/'+pdb+'/' ## +pdb+'_'+chain+'.pdb'
                os.system(split_pdb)
