...
table = pd.read_csv('/home/takacsg/Documents/sh2db/data/SH2_domain_containing_prot_right_resnum_with_pdb_nums.csv', engine ='python')
print(table.head())
            ...
            for chain in table[table["PDB ID"] == pdb]["PDB chain ID"].unique():
                for i in table[table['PDB ID'] == pdb]["New_PDB_start"].unique():
                    for j in table[table['PDB ID'] == pdb]["New_PDB_stop"].unique():
                        ...
                        split_SH2D = 'pdb_selres -'+str(i)+':'+str(j)+' /home/takacsg/Documents/sh2db/Structures/'+cat+'/'+gene+'/'+pdb+'/'+pdb+'_'+chain+'.pdb > /home/takacsg/Documents/sh2db/Structures/'+cat+'/'+gene+'/'+pdb+'/'+pdb+'_'+chain+'_SH2.pdb'
